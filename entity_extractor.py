...
from ac import load_cache, norm

"""
    A comprehensive biological entity mention extractor that combines:
    - AC (Aho-Corasick) automaton for exact matching
    - Regex-based pattern matching for enzymes and explicit IDs
...
class BioEntityExtractor:
    ...
    def __init__(self, ac_automaton, alias_map: Dict, llm_extractor=None, default_fuzzy_threshold: int = 88):
        """
        Initialize the extractor with AC automaton and alias mapping.
        
        Args:
            ac_automaton: Aho-Corasick automaton for exact matching
            alias_map: Dictionary mapping normalized text to entity candidates
            default_fuzzy_threshold: Default threshold for fuzzy matching (0-100)
        """
        self.ac_automaton = ac_automaton
        self.alias_map = alias_map
        self.vocab = list(alias_map.keys())
        self.default_fuzzy_threshold = default_fuzzy_threshold
        self.llm_entity_extractor = llm_extractor
    
    @classmethod
    def from_cache(cls, cache_path: str, default_fuzzy_threshold: int = 88):
        ac_automaton, alias_map = load_cache(cache_path)
        return cls(ac_automaton, alias_map, default_fuzzy_threshold)
    ...
    def extract_mentions(
        ...
        """
        Extract all biological entity mentions from the given question.
        
        Args:
            question: Input text to extract entities from
            species_hint: Optional species code to filter results (e.g., 'ath', 'gmx')
            use_regex: Whether to use regex-based extraction
            use_llm: Whether to use LLM-based extraction
            fuzzy_threshold: Fuzzy matching threshold (uses default if None)
            
        Returns:
            List of entity mention dictionaries with keys: text, id, db, species, start, end, src, confidence
        """
